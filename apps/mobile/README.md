...
No `apply_*.py` CI patch step was needed for this one — `@capgo/capacitor-alarm`'s own Android module carries whatever manifest entries it needs via normal Gradle manifest merging (its own `AndroidManifest.xml` declares no extra permissions itself; `ACTION_SET_ALARM` is a standard implicit intent handed to the OS, not something the *calling* app needs special permission for), so it just needs to be present at `npx cap sync android` time like `@capacitor/push-notifications` already is — no native source file to regenerate on every build. Verified so far: it builds cleanly in CI (`./gradlew assembleDebug`/`bundleRelease` succeed with the dependency present). **Not yet verified:** whether `skipUi` actually skips the Clock app's confirmation screen on a real device, and whether the resulting alarm rings/bypasses silent mode as expected — that only gets confirmed once a build with this dependency reaches a real phone.
