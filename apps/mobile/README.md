...
## Wake-up alarm (2026-09-02)

Unlike every notification feature above, this one is deliberately **not** built as another custom native class. A genuinely loud, silent-mode-bypassing, full-screen alarm is exactly what Android's own Clock app already does — reimplementing that (a custom `AlarmManager` schedule, a full-screen ringing `Activity`, `STREAM_ALARM` audio, boot-safe rescheduling) would be the single largest native surface in this app for a worse, less-tested version of something the OS already ships. Instead, `apps/web/src/app/notifications/page.tsx`'s new "Wake up at" field uses [`@capgo/capacitor-alarm`](https://github.com/Cap-go/capacitor-alarm) — an actively maintained plugin that hands the request to the device's own Clock app via Android's `AlarmClock` intents (iOS 16+/26+ uses Apple's `AlarmKit` the same way, though this repo has no iOS build yet — see the iOS section above).

The real, honest limitation, not glossed over: Android's system doesn't let an app query, update, or cancel an alarm it asked the Clock app to create (`getAlarms()`/`cancelAlarm()` both explicitly return "not supported" on Android in this plugin — see its own type definitions). So this app only ever *sends a request* to add an alarm; it can't later silently correct or remove one on the phone's behalf. `notifications/page.tsx` only re-sends that request when the wake-up time preference actually changes (not on every unrelated preference save, to avoid piling up duplicate alarms), and `skipUi: true` asks the Clock app to add it without its own confirmation screen — which isn't guaranteed on every OEM Clock app, so the save flow reports back honestly whether the phone confirmed it rather than assuming success.

**Correction (2026-09-03) — the paragraph below was wrong.** It originally said no `apply_*.py` CI patch step was needed here, on the theory that `ACTION_SET_ALARM` is just a standard implicit intent and the *calling* app needs no special permission for it. A real Pixel running this app proved that wrong the first time a real device actually tried it: tapping "Set alarm on this phone" surfaced a raw Android error in the save-status message, `Permission Denial: starting Intent { act=android.intent.action.SET_ALARM ... } requires com.android.alarm.permission.SET_ALARM`. Google's own stock Clock app (`com.google.android.deskclock`) enforces that the caller hold `com.android.alarm.permission.SET_ALARM` before it will accept an `ACTION_SET_ALARM` intent at all — it's a *normal* permission (no runtime consent dialog, unlike location/camera/etc.), but it still has to be declared in the calling app's manifest, and this app's manifest never declared it. `@capgo/capacitor-alarm`'s own bundled `AndroidManifest.xml` is empty (confirmed by reading it directly) — it declares no permissions of its own, so this was always on the calling app to add, and nothing did. Fixed by `scripts/apply_alarm_permission.py`, a new CI patch step in the same family as the others in this file (same "re-apply after every fresh `cap add android` scaffold" reasoning, since `android/` isn't committed) — see that script's own module docstring for the full story.

Original paragraph, kept for the record rather than deleted, since it's what generated the bug: "No `apply_*.py` CI patch step was needed for this one — `@capgo/capacitor-alarm`'s own Android module carries whatever manifest entries it needs via normal Gradle manifest merging (its own `AndroidManifest.xml` declares no extra permissions itself; `ACTION_SET_ALARM` is a standard implicit intent handed to the OS, not something the *calling* app needs special permission for), so it just needs to be present at `npx cap sync android` time like `@capacitor/push-notifications` already is — no native source file to regenerate on every build."

Verified so far: it builds cleanly in CI (`./gradlew assembleDebug`/`bundleRelease` succeed with the dependency present and the permission now declared), and the missing-permission fix has been verified locally against a real, freshly-scaffolded Android project (both the fresh-scaffold and idempotent-rerun paths of the patch script). **Not yet verified:** that the fix actually resolves the error on a real device (needs a new build installed and re-tested), whether `skipUi` actually skips the Clock app's confirmation screen on a real device, and whether the resulting alarm rings/bypasses silent mode as expected — those only get confirmed once a build with this permission fix reaches a real phone.
