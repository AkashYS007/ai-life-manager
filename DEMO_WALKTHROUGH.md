...
**Talking point:** this closes a real MVP requirement the PRD calls out explicitly (§10/§14) — not a nice-to-have added late, a gap that existed since day one of the frontend. Worth being direct if asked "so is there an iPhone app" — no, and there structurally can't be one built this way for iOS specifically (needs a Mac with Xcode, plus an Apple Developer account for real distribution — neither available in this build process). **Correction (2026-08-25, this section was stale):** there IS now a real native Android app — a Capacitor shell wrapping the live production site, with genuine OS-level push (Firebase Cloud Messaging, works fully backgrounded), native voice-read notifications, and a home-screen icon, built via GitHub Actions and currently sideload-only by the user's own explicit choice (not a Play Store listing). The installed PWA remains the realistic *iOS* mobile answer for now, and is genuinely close to a native feel once installed — the Android answer is a real native shell. Also worth naming the scope honestly: offline support covers exactly the three actions the PRD names — adding a task, completing a task, journaling — not the whole app; anything else (editing a task's full details, generating an AI plan, chat) still needs a live connection and will show its normal error state offline.
...
**Talking point:** before the original version of this increment, "delivery" meant nothing more than the notification sitting there waiting for someone to open the app — now it's real Web Push (the standard, VAPID-based mechanism a browser-based PWA can use — no native FCM/APNs SDK or app-store account needed, matching what this app actually is), real email through Resend's API, and now real SMS through Twilio's API too — all three channels the PRD names actually send something now. Worth naming the honest scope, **updated 2026-08-25 — this used to say no retry existed at all, which is no longer true:** each channel now gets a bounded in-process retry with backoff (a few real extra attempts over a couple of seconds) before falling through to logged-and-marked-attempted, so a brief transient failure (a momentary 5xx from Resend/Twilio) now genuinely recovers instead of failing on the first try. What's still true: this is retry, not a durable job queue — an outage longer than that short retry window still permanently downgrades that one notification to in-app-only, with no later automatic re-attempt (see the project's own change log for the current thinking on closing that specific gap without standing up new infrastructure). The in-app Notifications page remains the fallback of last resort no matter what happens with any of the three channels — nothing here can make a notification vanish without a trace.
...
- Real push, email, and now SMS delivery are all wired up (Web Push, Resend, Twilio) — every channel the PRD names now actually sends something, not just a saved preference. Delivery now gets a bounded in-process retry-with-backoff per channel (updated 2026-08-25 — this used to say no retry existed), but is still not a durable job queue: an outage longer than that short retry window still permanently downgrades to in-app-only. SMS specifically has never sent a real text in this build process (no Twilio credentials available here) — mirrors the already-working email code closely, but still worth testing with a real Twilio trial account before fully trusting it.
...
- **Corrected 2026-08-25 — this used to say no native app existed at all, for either platform:** there is now a real native Android app (Capacitor shell around the live production site — genuine OS-level FCM push, native voice-read notifications, home-screen icon; sideload-only by the user's own choice, no Play Store listing). No native iOS app — that still needs a Mac with Xcode and an Apple Developer account this build process doesn't have; the installed PWA remains the realistic iOS mobile story for now.
